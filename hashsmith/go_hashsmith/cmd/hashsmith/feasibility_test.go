...
func TestForceStartsARefusedRun(t *testing.T) {
	...
	out, err := captureStderrResult(t, func() error {
		return checkFeasibility(impossibleMaskKeyspace, false, "bcrypt", target, "", "prefix", 4, true, nil, 0)
	})
	if err != nil {
...
func TestFeasibilityNeverRefusesWhatItCannotMeasure(t *testing.T) {
	...
	t.Run("unknown keyspace", func(t *testing.T) {
		out, err := captureStderrResult(t, func() error {
			...
			return checkFeasibility(-1, false, "bcrypt", target, "", "prefix", 4, false, nil, 0)
		})
		if err != nil {
	...
	t.Run("saturated keyspace", func(t *testing.T) {
		out, err := captureStderrResult(t, func() error {
			return checkFeasibility(math.MaxInt64, false, "bcrypt", target, "", "prefix", 4, false, nil, 0)
		})
		if err != nil {
	...
	t.Run("empty keyspace", func(t *testing.T) {
		out, err := captureStderrResult(t, func() error {
			return checkFeasibility(0, false, "bcrypt", target, "", "prefix", 4, false, nil, 0)
		})
		if err != nil {
...
func TestFeasibilityProbeIsNotAStartupTax(t *testing.T) {
	...
	start := time.Now()
	rate, ok := feasibilityRate(1_000_000, "md5", target, "", "prefix", 4, nil, 0)
