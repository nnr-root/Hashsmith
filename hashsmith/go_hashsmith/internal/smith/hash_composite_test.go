...
func TestCompositeConstructionVectors(t *testing.T) {
	...
	// saltOverride is for constructions whose salt is not free-form text. The
	// hex-salt forms decode their salt before hashing, so "Zx9" is not a legal
	// input for them — and making the decoder fall back to the raw bytes
	// instead would re-create exactly the silent mismatch these exist to avoid.
	cases := []struct{ name, want, saltOverride string }{
		...
		{"sha256-utf16lepass-salt", "dcdda1e5ef82d2610cdfa216d185623ac3efae3a23e05d87bfb648ff041de5a4", ""},
		{"sha256-utf16lepass-hexsalt", "4160e19679942328b8a3773c9658229910abfa4168e04f964518d2019ef55c86", "5a7839ab"},
	}
	...
	for _, c := range cases {
		effSalt := salt
		if c.saltOverride != "" {
			effSalt = c.saltOverride
		}
		got, err := hashText(pass, c.name, effSalt, "prefix")
		...
		// The explicit -s form.
		if ok, err := verifyCandidate(pass, c.want, c.name, effSalt, "prefix"); err != nil || !ok {
