...
// Main runs the command-line interface: it reads os.Args, does the work and
// calls os.Exit. It is what cmd/hashsmith invokes and nothing else should.
//
// A library caller wants the hashsmith package at the module root instead.
// This function owns the process — it exits, it writes to stdout, it prints a
// banner — and none of that belongs inside someone else's program.
func Main() {
	rawArgs := os.Args[1:]
...
func printHelp() {
	...
	fmt.Println("  wordlists     [--scan] [--set-default <path>] [--clear-default]")
	...
	fmt.Println("            $HASHSMITH_WORDLIST pins one for a shell, `wordlists --set-default` pins one permanently,")
	fmt.Println("            --no-auto-wordlist forces the built-in common.txt,")
